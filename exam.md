...
1. **企業分析文字檔案型的非結構化資料時，可以用文字探勘（text mining）的工具及搭配情感性分析（sentiment analysis），請問什麼是文字探勘與情感性分析？ ```15%, 105年```**

1. **而網頁探勘（web mining）主要是要了解什麼？請詳述之。 ```10%, 105年```**


1. **何謂 IT 消費化與 BYOD（Bring Your Own Device）？ ```10%, 105年```**
1. **IT 消費化迫使企業必須要做出什麼樣的調整？ ```15%, 105年```**

1. **何謂資訊科技服務管理（Information Technology Service Management, ITSM）？ ```10%, 105年```**
1. **ITSM 有那四個要素？請針對這四個要素詳述之。 ```15%, 105年```**

1. **網路安全系統的第二道防線是入侵偵測系統（Intrusion Detection System, IDS），請詳述之。 ```10%, 105年```**
1. **入侵偵測系統（Intrusion Detection System, IDS）依照資料收集方式的不同可分為那三種類型？請詳述之。 ```15%, 105年```**
...
1. **試說明下列關於隱私權的問題：**
	- 何謂隱私權？ ```5%, 105年```
	- 對隱私權侵犯的型態有那些？ ```10%, 105年```
	- 我國對隱私權保護主要法律有那些及其主要意涵或精神為何？ ```15%, 105年```

---

# 鐵路(6月)
## 資訊管理(高員三級)
1. **請分別由以下理論的角度，說明資訊系統對組織的影響 ```25%, 106年```**
...
1. **某銀行遭駭客以社交工程（Social Engineering）入侵，致使自動提款機 ATM 不正常吐鈔達數千萬元。請說明：**
	- 社交工程及其常採取的方法？ ```15%, 106年```
	- 機關如何進行社交工程資安演練？ ```10%, 106年```
...
1. **資料庫管理系統（Database Management Systems）改善了傳統檔案系統（File Systems）的那些缺點？ ```25%, 106年```**
...
1. **回答下列問題：**
	- 請定義雲端運算（Cloud computing）的意義並說明其範圍。 ```20%, 105年```
	- 雲端運算對使用電腦需求者有何影響？ ```10%, 105年```

1. **回答下列問題：**
	- 何謂資訊倫理（Information ethics）？（13 分）
	- 請基於資訊倫理，說明對隱私權的看法。 ```12%, 105年```

1. **回答下列問題：**
	- 何謂 CNS 27005 風險管理（Risk Management）？ ```11%, 105年```
	- 請以兩個例子說明風險管理的運作。 ```14%, 105年```

## 資料通訊(高員三級)
- **答下列問題： ```25%, 106年```**
	- 網際網路最新的外部路由通訊協定（Exterior Routing Protocol）為何？
	- 什麼是 4G LTE？
	- 無線 Ethernet 網路中所使用的媒體存取控制（MAC）方法為何？
	- 試定義 ARP。
	- 什麼是 Wi-Fi？

- **回答下列有關網路實體層（Physical Layer）之問題：**
	- 試定義“Bit Rate”與“Symbol Rate”；並比較其差異性。 ```15%, 106年```
	- 試列出網路通訊使用數位傳輸（Digital Transmission）之五項優點。 ```10%, 106年```

- **試定義網際網路中之“Server Name Resolution”；此外，並說明使用何種機制執行此工作？ ```25%, 106年```**

- **下列每一通訊協定是屬於 TCP/IP 網路中那一層次（Layer）架構？此外，並說明每一通訊協定之英文全寫與主要的功能。 ```15%, 106年```**
	- TFTP
	- SMTP
	- IGMP

- **什麼是 FCS？產生器之多項式（Generator polynomial）為 X^4 + X^3 + 1 以及欲傳送之訊息位元內容為 10100001，試利用 CRC 計算出 FCS 為何？ ```25%, 106年```**
...
- **請回答下列相關問題： ```20%, 105年```**
	- ping 指令是用來診斷網路連線狀態與連線品質，若網路品質監測公用程式 ping 指令之執行如下(a)及(b)所示兩種結果，請問其所呈現的意義為何？試詳細說明之。
...
- **網路安全已是網路架構的標準配備，在公眾網路傳送或儲存在資料庫裡的資料，無論是商業機密，或是個人隱私資料，皆必須做好網路安全的工作，請回答下列相關問題： ```20%, 105年```**
	- 為什麼在網路系統中，需要進行使用者身分（Identity）及鑑別（Authentication）？試詳細說明之。
	- 對於基本的加解密系統中，為什麼加密方法或演算法需要公開？試說明之。

- **TCP/IP 是一種網路通信協定，目前已成為一項國際標準協定，請回答下列相關問題： ```20%, 105年```**
	- TCP/IP 主要功能為何？細分幾個層級（Layer）？試繪圖說明之。
	- IP 主要功能及 IP 資料頭格式包括那些重要欄位？IP 封包中的 port number 欄位用途為何？試加以詳細說明之。

- **網路對於生活影響程度與日俱增，駭客利用網路從事電腦犯罪也屢見不鮮，請回答下列相關問題： ```20%, 105年```**
	- 何謂分散式阻斷服務攻擊（DDoS）？試說明之。
	- 常見的網路系統安全威脅來自那三個來源？試說明之。
...
- **資訊系統雖然很有用，但需要付出成本才得以建置。請說明：**
	- 組織開發與建置資訊系統所須付出的成本包括那些？ ```10%, 106年```
	- 資訊系統可以為組織帶來那些效益？ ```15%, 106年```

- **在網路盛行的時代，許多人都會在網路搜尋相關資料。但這其中，經常收到大量的資料，反倒出現資訊過載（information overload）的現象。請說明：**
	- 為何會出現資訊過載的現象？ ```10%, 106年```
	- 有那些方案可以減低資訊過載的現象？ ```15%, 106年```

- **在面對日益增加的網路服務上，為確保能達到資訊安全所要求的機密性（confidentiality）、完整性（integrity）、可稽核性（accountability）、確認性（authentication）及不可否認性（non-repudiation）等基本需求。請分別說明其意義以及可以採取那些措施以分別達到其目的。 ```25%, 106年```**
...
1. **一般網路位址分配指派分為 CIDR 與 Classful Addressing。 ```20%, 105年```**
	- 請說明 CIDR 是如何分配網路位址區塊？
	- 請說明 Classful Addressing 是如何分配網路位址區塊？
...
1. **請回答下列有關資料探勘問題：**
	- 資料探勘有那幾種模型？請說明每個模型。 ```15%, 106年```
		...
		品，隔天後出現腹瀉的機率是45%。
	- 假設老闆要您分析顧客消費行為，您將會採取那種模型？說明其原因。 ```10%, 106年```

1. **請回答下列有關電子商務方面問題：**
	- 請說明搜尋引擎行銷（Search Engine Marketing, SEM）。 ```10%, 106年```
	- 智慧型手機以及行動上網的普及，使得電子商務的經營模式除了傳統的 B2B、B2C、C2B、C2C 之外，更產生 O2O 的模式，請說明何謂 O2O，並舉出兩個應用案例。 ```15%, 106年```
...
1. **請說明防火牆（firewall）之目的為何？為達到此目的，其利用的原理或方法為何？ ```10%, 105年```**
1. **假如有個網路攻擊者（attacker），有辦法入侵你區域的網域名稱系統（DNS: domain name system）伺服器，那它可以用什麼方式，來竊取你銀行的帳號及密碼資訊？ ```10%, 105年```**
...
1. **SSL（Secure Socket Layer）安全協定目前被廣泛應用在 HTTP 連線，以達成電子商務應用的安全性，詳細說明 Web Server SSL 的運作原理。 ```20%, 106年```**

1. **解釋下列常見的入侵偵測技術： ```20%, 105年```**
	- 異常行為入侵偵測（Anomaly Intrusion Detection）。
	- 錯誤行為入侵偵測（Misuse Intrusion Detection）。

1. **作業系統的存取控制（Access Control）模式分為那幾種類型？並詳細說明之。 ```20%, 105年```**

1. **DNS（Domain Name Service）是網際網路的重要基礎服務，近年來已成為 DDoS（Distributed Denial of Service）網路攻擊的目標之一，詳細說明 DNS 放大攻擊（DNS Amplification Attack）的原理及其防護方法。 ```20%, 105年```**
...
1. **以下兩題 ```20%, 106年```**
	- 比較資料鏈結層（data link layer）與傳輸層（transport）的功能特性，各舉出兩個相同點，以及兩個不同點。
	- 為什麼路由（routing）的計算，是網路層（network layer）的工作，而不是在傳輸層或資料鏈結層？

1. **以下兩題 ```20%, 106年```**
	- 假設在一個 slotted ALOHA 網路裡，只有三個工作站 A, B, C。而每個工作站在一個時槽（time slot）裡會傳送訊框（frame）的機率，分別是 PA = 0.2，PB = 0.3，PC = 0.4。
		- 則工作站 A 的吞吐量（throughput）約是多少？
		- 整個系統的吞吐量又是多少？
	- 請比較 WiFi 及藍牙（Bluetooth）的媒介存取協定（medium access control protocol）主要的不同點？至少舉出兩點。

1. **以下兩題**
	- 比較 pure ALOHA、non-persistent CSMA、1-persistent CSMA、p-persistent CSMA之間的不同。 ```13%, 106年```
	- CSMA/CA 的 CA（Collision Avoidance）碰撞避免的策略為何？舉兩個策略並說明之。 ```12%, 106年```
