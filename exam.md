...
1. **企業分析文字檔案型的非結構化資料時，可以用文字探勘（text mining）的工具及搭配情感性分析（sentiment analysis），請問什麼是文字探勘與情感性分析？ ```15%, 105年關務三等```**
...
1. **什麼是 VPN？```8%, 106年```**
	- 一種常用於連線中、大型企業或團體與團體間的私人網路的通訊方法。虛擬私人網路的訊息透過公用的網路架構來傳送內部網的網路訊息。它利用已加密的通道協議（Tunneling Protocol）來達到保密、傳送端認證、訊息準確性等私人訊息安全效果。這種技術可以用不安全的網路來傳送可靠、安全的訊息。
...
1. **下列有關 TCP/IP 網路中通訊協定是屬於那一層次（Layer）架構？並說明每一通訊協定之主要的功能含英文全寫。 ```20%, 106年```**
	...
	1. SNMP
		...
		- 其構成了網際網路工程工作小組（IETF，Internet Engineering Task Force）定義的Internet協定族的一部分。該協定能夠支援網路管理系統，用以監測連線到網路上的裝置是否有任何引起管理上關注的情況。它由一組網路管理的標準組成，包含一個應用層協議（application layer protocol）、資料庫模式（database schema），和一組資料物件。
	1. OSPF
...
1. **ping 和 traceroute 常用來檢視主機的網路連線狀態。**
	...
	1. 請說明 traceroute 的功能。 ```4%, 105年```
		- 是一種電腦網路工具。它可顯示封包在 IP 網路經過的路由器的 IP 位址。
...
1. **請說明何謂跨網站指令碼（XSS，Cross-Site Scripting）攻擊，以及如何防範此攻擊。 ```25%, 106年```**
	- 是一種網站應用程式的安全漏洞攻擊，是代碼注入的一種。它允許惡意使用者將程式碼注入到網頁上，其他使用者在觀看網頁時就會受到影響。這類攻擊通常包含了 HTML 以及使用者端腳本語言
...
1. **某銀行遭駭客以社交工程（Social Engineering）入侵，致使自動提款機 ATM 不正常吐鈔達數千萬元。請說明：**
	1. 社交工程及其常採取的方法？ ```15%, 106年```
	1. 機關如何進行社交工程資安演練？ ```10%, 106年```

1. **麥可．波特（Michael Porter）利用五力分析模型（Five Forces Model）進行產業分析。請說明**
	1. 五力分析模型？ ```10%, 106年```
	1. 組織可以採取那些競爭策略？ ```6%, 106年```
	1. 此模型有那些缺陷？ ```9%, 106年```
...
1. **回答下列問題：**
	1. 何謂資訊倫理（Information ethics）？（13 分）
...
1. **回答下列有關網路實體層（Physical Layer）之問題：**
...
1. **防火牆（Firewall）是一般組織最常見的安全防護工具，而現代防火牆多採用 Stateful Packet Inspection（SPI）技術，請詳細回答下列問題： ```20%, 105年```**
	1. 何謂 SPI？為何要使用此技術？
		- 狀態防火牆是一種能夠提供狀態封包檢查或狀態檢視（stateful inspection）功能的防火牆，能夠持續追蹤穿過這個防火牆的各種網路連線的狀態。這種防火牆被設計來區分不同連線種類下的合法封包。只有符合主動連線的封包才能夠被允許穿過防火牆，其他的封包都會被拒絕。
	1. Packet-filtering Firewall 和 Web Application Firewall（WAF）有何不同？

1. **無線區域網路（Wireless LAN）已是目前非常普遍的網路技術，請問： ```10%, 105年```**
	1. 為何無線區域網路在實體層要使用展頻（Spread spectrum）技術？
	1. Evil Twins Access Point 攻擊會對 WLAN 造成何種安全危害？請說明之。
